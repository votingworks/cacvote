...
pub(crate) async fn sync(
    ...
    let sync_input = RaveServerSyncInput {
        ...
        elections: db::get_elections_to_sync_to_cacvote_server(executor)
            ...
            .map_err(|e| {
                color_eyre::Report::msg(format!(
                    "failed to get elections to sync to CACVote Server: {}",
                    e
                ))
            })?,
        registration_requests: db::get_registration_requests_to_sync_to_cacvote_server(executor)
            ...
            .map_err(|e| {
                color_eyre::Report::msg(format!(
                    "failed to get registration requests to sync to CACVote Server: {}",
                    e
                ))
            })?,
        registrations: db::get_registrations_to_sync_to_cacvote_server(executor)
            ...
            .map_err(|e| {
                color_eyre::Report::msg(format!(
                    "failed to get registrations to sync to CACVote Server: {}",
                    e
                ))
            })?,
        printed_ballots: db::get_printed_ballots_to_sync_to_cacvote_server(executor)
            ...
            .map_err(|e| {
                color_eyre::Report::msg(format!(
                    "failed to get printed ballots to sync to CACVote Server: {}",
                    e
                ))
            })?,
        scanned_ballots: db::get_scanned_ballots_to_sync_to_cacvote_server(executor)
            ...
            .map_err(|e| {
                color_eyre::Report::msg(format!(
                    "failed to get scanned ballots to sync to CACVote Server: {}",
                    e
                ))
            })?,
    };
    ...
    client
        ...
        .map_err(|e| {
            color_eyre::eyre::eyre!(
                "CACVote Server responded with an error (status URL={}): {}",
                endpoint,
                e
            )
        })
