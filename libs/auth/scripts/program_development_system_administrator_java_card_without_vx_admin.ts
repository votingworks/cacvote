...
import { DEV_VX_CERT_AUTHORITY_CERT_PATH } from '../src/config';
...
async function instantiateJavaCardWithOneOffVxAdminPrivateKeyAndCertAuthorityCert(
  ...
  const vxAdminCertAuthorityCert = await createCert({
    ...
    signingCertAuthorityCertPath: DEV_VX_CERT_AUTHORITY_CERT_PATH,
  ...
  const card = new JavaCard({
    ...
    vxCertAuthorityCertPath: DEV_VX_CERT_AUTHORITY_CERT_PATH,
  });
  await fs.writeFile(
    'vx-admin-cert-authority-cert.pem',
