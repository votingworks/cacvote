...
test('CardCommand with data requiring multiple APDUs', () => {
  ...
  expect(command.asCommandApdus().map((apdu) => apdu.asBuffer())).toEqual([
    Buffer.concat([
      Buffer.from([0x10, 0x01, 0x02, 0x03, 0xff]),
      Buffer.alloc(200, 1),
      Buffer.alloc(55, 2),
    ]),
    Buffer.concat([
...
test.each<{
...
}>([
  {
    ...
    tlv: Buffer.concat([Buffer.from([0x01, 0x7f]), Buffer.alloc(127)]),
    expectedOutput: [
      Buffer.from([0x01]),
      Buffer.from([0x7f]),
      Buffer.alloc(127),
  ...
  {
    tagAsByteOrBuffer: 0x01,
    tlv: Buffer.concat([Buffer.from([0x01, 0x81, 0xff]), Buffer.alloc(255)]),
    expectedOutput: [
      Buffer.from([0x01]),
  ...
  },
  {
    tagAsByteOrBuffer: 0x01,
    tlv: Buffer.concat([
      Buffer.from([0x01, 0x82, 0xff, 0xff]),
      Buffer.alloc(65535),
    ]),
    expectedOutput: [
      Buffer.from([0x01]),
